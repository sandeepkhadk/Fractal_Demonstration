...
# settings
res = width, height = 800,600 # with modern video card with CUDA support - increase res '1600, 900' and set 'ti.init(arch=ti.cuda)'
        ...
        while running:
            ...
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    running = False  # Exit the loop gracefully
                elif event.type == pg.KEYDOWN and event.key == pg.K_RETURN:
                    running = False  # Exit after Enter key is pressed

            # Update FPS display
            self.clock.tick()
            pg.display.set_caption(f'FPS: {self.clock.get_fps() :.2f}')

        # Show the continue/exit menu after the main loop
